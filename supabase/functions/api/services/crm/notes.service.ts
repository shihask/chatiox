...
import type { CreateNoteInput, ListNotesQuery } from '../../schemas/crm/notes.schemas.ts'
import type { NoteDTO } from '../../dtos/crm/notes.dtos.ts'

export async function listNotesByContact(ctx: WorkspaceRequestContext, contactId: string): Promise<NoteDTO[]> {
  const rows = await notesRepository.listByContact(ctx.supabase, ctx.workspaceId, contactId)
  return rows.map(mapNoteRowToDTO)
}

export async function listNotes(ctx: WorkspaceRequestContext, query: ListNotesQuery): Promise<Page<NoteDTO>> {
  const page = await notesRepository.list(ctx.supabase, ctx.workspaceId, query)
  return { ...page, items: page.items.map(mapNoteRowToDTO) }
}
